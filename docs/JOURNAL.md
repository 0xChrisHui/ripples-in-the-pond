...
- **K4 投影改"物理软投影"模型（用户要把"层级差"在投影里充分显现）**：旧版 `rise=clamp(air/0.12)` 在 air=0.12 早饱和 → 差 15 层/30 层的球影几乎一样。新版按**悬空物在水面投影的物理因素**重设计（`computeShadowMask`）。完整因素清单（供后续调参/扩展参考）：
  ...
  - **落地取核心 ⑥⑦⑧⑨⑮⑯⑰**：`t=clamp(air/0.5)`(不再早饱和)；视差 offset∝真实 air；外半影 `r·(1+1.6·t·g)` 涨、本影 `(1-t)·0.85` 缩(高→全半影=糊)；折射模糊 `warp=grad·(1+6·t·g)`；衰减 `peak=mix(1,0.3,t)`(淡但不消失)；再被移动水光网纹打碎。`g=uShadowHeight`(新滑块"投影高度感"默认 1.2)=高度影响总增益。
