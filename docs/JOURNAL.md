...
- **生产结果**：新钱包 `0x456b...7708` 自动收到 ECHO #1；metadata 与 mint 均一次成功，40 个永久对象全部达到至少 2/3 网关 quorum，链/DB/metadata/Score source 和双视口 36/36 长播通过。
...
## 2026-09-21 — P15-I 条件流式与信任边界

- **I4 被真实数据触发**：I3 Preview 点击到首声 cold p95 为 `3717ms`，超过 2 秒 Gate；因此底曲允许从已发布、不可覆盖且完成全字节 attestation 的 Verified Edge URL 直接流式起播，首声前不再等待客户端整文件 SHA-256。
- **永久回退仍完整校验**：音效仍在客户端逐字节校验；流式底曲发生媒体错误时，播放器暂停在媒体时钟位置，下载永久底曲并完成 SHA-256 后再解码续播，Permanent Core 仍可脱离 Edge 恢复。
- **不采用页面加载静音播放**：实测会让底曲与 8 秒启动音效闭包争抢连接，降低成功率并制造长尾；该实验独立提交后已回退。稳定 I4 Preview 20/20 有效，cold 点击到首声 p95 `1775ms`，hot `1814ms`；热预算的剩余瓶颈是 HKG Blob 首包延迟，不在本轨新增 CDN 架构。

## 2026-09-22 — P11 作品页日食运动

- **复用首页力场而非新增动画层**：作品页播放时解除独球钉点、启用首页同源浮动与漂移，并暂时移除单节点中心力；停止后用 1050ms 缓动把日食带回唱片中心，减少动态效果模式则立即归位。

## 2026-09-22 — 全播放器进度条支持拖动

- **交互决定**：站内播放器采用 Radix Slider 的指针捕获模型和 Plyr 的常显手柄，不再使用透明原生 range；拖动期间只预览、松手后一次性 seek，并让全局播放器在提交同一帧同步显示位置，避免 250ms 轮询旧值造成回弹。顶部播放器允许手柄越过边框完整显示。永久 Decoder 保持自包含，只修改未来版本源码，不修改或重传历史永久资产。

## 2026-09-24 — P11-I3 路由转场收场条件

- **由入场层收场**：参考实现用离场层（首页渐隐）的 `transitionend` 收场；实测该事件在渐隐并隐藏的元素上会被延迟到下一次相位变化才补发。改为只由完全显现的入场层（进入档案看路由层、回首页看首页层）且计算透明度为 1 时收场，并保留“地址落地 + 时长 + 120ms”兜底。
- **焦点与离场层**：离场的首页层加 `inert`（参考实现只有 aria-hidden 与 pointer-events，Tab 仍能进入）；收场时若焦点被 inert 丢回 body，交给目标页 `data-pond-focus-entry` 入口。
- **历史导航**：`popstate` 时若地址监听已先进入 `leaving-home`，保持该相位而不是跳到终态，否则浏览器前进既无淡入也不交接焦点。

## 2026-09-24 — P11-I4 圆圈进退场停止点

- **连续性已成立**：路由往返没有重建 Water Core、Canvas 或音乐圆节点；35 个普通圆 ID、既有涟漪和播放状态都能跨 `/me` 保留。
- **动画目标尚不可靠**：由被动 effect 启动 presence 动画时，旧 archive 写入会越过快速反向边界，在 `entering-home` 中途把值拉到 0。稳定终态正确不能替代 0/120/260/520ms 的过程 Gate。
- **执行决定**：三次动态 Gate 均失败后按夜间规则停止，I4 不提交，I5–I7 不启动。下一次从 effect 时序与过期 RAF 写入保护开始，不重复排查字体、节点连续性或播放连续性。
- **恢复结论**：用户授权继续后，presence 改用 layout effect 并给 RAF 加 generation；硬件 Edge 证明正常离场、回场与中途反向。SwiftShader 的极低帧率单独记录为测试环境问题，不再放宽产品断言来迁就它。

## 2026-09-24 — P11-I5 私人档案预备边界

- **唯一实例**：首页与 `/me` 共用一个提前挂载的档案实例；路由只改变可见性与交互权，不再为正式页或测试页创建第二个 PondGL。
...
## 2026-09-24 — P11-I7 档案与作品锚点边界

- **稳定身份才增强**：只有 `success + tokenId` 的唱片行进入命名锚点；processing、failed、直接访问和来源失效保留普通链接与页面淡入。
- **唯一活动来源**：同页多条唱片中只有当前点击行拥有 `score-record`，来源记录绑定账号和单调导航代次，旧异步回调不能覆盖快速改点的新目标。
- **常驻档案恢复**：来源只有在确认到达匹配 Score 后才能反向；返回 `/me` 后按最新数据重新定位分区与页码，再恢复滚动和焦点。账号变化或行消失立即降级。
- **失败终态**：ready 来源若落到真实 failed/fallback 页面，自动回到原档案行并恢复交互；Escape 与动画中反向也清理 pending，不把半张唱片留在页面上。

## 2026-09-22 — P11 永续水塘边界（旧分支迁入）

- **水面是布局，不是页面背景**：`/`、`/me`、`/score/[id]` 共用一棵 route-group Water Core；路由只交换前景 Scene 与交互所有权。Score 音频/P9、首页圆圈和私人档案数据继续各自隔离，`/test3` 不加入生产共享生命周期。

## 2026-09-22 — P11 档案到作品的连续锚点（旧分支迁入）

- **只变形真实 ready 唱片**：档案行仅在拥有稳定 Token 身份时保存来源并启用 View Transition；processing、failed、直接访问与来源失效都退回普通 Surface 淡入，不伪造唱片身份。
- **等待边界只包前景**：路由内容的 Suspense 不得包住 Water Core 或转场控制器；否则并发导航会冻结持续水面。动画阶段使用同步 UI 提交，路由加载只影响前景页面。
- **转场收场以目标路由落地为准**：旧地址上的兜底计时器和子元素冒泡的透明度事件都不能结束圆圈显隐；否则冷路由会反向闪回，热路由却看似正常。

## 2026-09-23 — P11 私人档案预备层试验（旧分支迁入）

- **按用户新决定提前准备私人档案**：推翻 G-D4 的「仅进入 `/me` 后请求」边界，在共享水塘内后台挂载唯一档案实例；身份隔离与既有缓存不变。首页点击只在档案三栏与唱片回声都已就绪或明确报错后启动换场，隐藏实例保持 inert，`/me` 路由不再重挂第二份档案。

## 2026-09-23 — P11 圆圈与档案退场同步（旧分支迁入）

- **水面圆圈和页面前景必须共用缓动曲线**：浏览器实测档案已显现约 98% 时，圆圈仍有约 40% 可见度；统一曲线并在稳定阶段归零，保留同一 WebGL Canvas，不靠延迟或遮罩掩盖。

## 2026-09-23 — 旧 Score 数字链接身份冻结（旧分支迁入）

- **公开读与本地铸造链解耦**：`/score/<tokenId>` 永远查询既有 OP 主网 ScoreNFT 的 verified snapshot，持有人与凭证也只指向主网；本地 Sepolia 可供测试，但不能凭同一个 tokenId 冒充旧唱片。

## 2026-09-23 — 私人唱片列表关联恢复（旧分支迁入）

- **保留原曲目归属**：P15 加入音频身份复合外键后，私人唱片列表仍按原 `track_id` 外键读取曲目标题；显式指定该关系，避免 PostgREST 在两条合法关系之间猜测。

## 2026-09-23 — Score 首播镜像路径补齐（旧分支迁入）

- **镜像默认值放在资源解析器**：调用方未显式传镜像地址时使用公开环境配置；显式空串仍保留只测 Arweave 的故障注入能力，继续执行哈希校验和永久网关回退。

## 2026-09-23 — P11-G/H 官网基线整合（旧基线分支迁入）

- **切换收口**：从 Score 返回档案时保留已预备的档案实例，仅在路由落地且目标可显示时清除导航意图；浏览器历史快速返回不得沿用旧档案 phase。
- **样式隔离**：Score 使用独立样式文件并由自身布局导入；Tailwind 只扫描 `app`/`src`，避免浏览器资料污染编译，不改变官网字体与日食设计。

## 2026-09-24 — P11-I 重放偏差与收口决定
