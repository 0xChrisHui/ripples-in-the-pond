...
### E048 — F7 Edge 长播脚本没有把被测页置前且先清理后写证据

- 📅 2026-09-11 / P14-F7 主网浏览器 Gate
- 😱 现象：首轮按钮在视口外而未真正点击；第二轮被测页被 Edge 扩展/同步页压到后台，时间轴严重节流；播完后又因 Windows 锁住临时 profile 卡在清理，JSON 尚未写出。
- 🧠 原因：CDP 物理点击前没有滚入视口；启动后没有显式创建唯一 target、bringToFront 与 focus；结果持久化错误地排在可能阻塞的临时目录清理之后。
- 🔧 修复：按钮先 `scrollIntoView` 再按中心坐标点击；禁用扩展、显式创建/置前页面；每个视口先原子写 partial JSON，最终报告与截图落盘后再做非阻塞清理。
- ✅ 结果：375 与 1440 两路均实时完成 1–36 段，约 271 秒后 ended；`4:29/4:29`、0 overflow、0 console/page error，总报告 PASS。
- 💡 长时间 E2E 必须先保存不可重建的运行 trace，再做可失败的环境清理；浏览器“窗口存在”也不等于被测 target 真正前台运行。

### E049 — 隔离发布工作树缺环境导致本地生产构建收集 API 失败

- 📅 2026-09-11 / P14-F7 收口验证
- 😱 报错：`Error: supabaseUrl is required`，构建在收集 `/api/artist/stats` 页面数据时退出。
- 🧠 原因：发布工作树为防止秘密落盘已删除 `.env.local`；首轮直接运行 `verify.sh`，构建进程没有 Supabase 环境。代码检查与 Forge 56/56 同轮均已通过。
...
### E053 — 本机构建因 Google Fonts 不可达误阻塞查询修复 Gate

- 📅 2026-09-24 / P11-I1-a
- 😱 现象：TypeScript、ESLint、P15 合同、生产只读回查与 Forge 56/56 均通过，但 `next build` 连续三次在五个 `next/font/google` 字体下载处失败。
- 🧠 原因：本机请求 `fonts.googleapis.com` 超时；Next.js 会在构建期下载并自托管 Google 字体，因此外部网络失败会中止生产编译，与本次 PostgREST 外键查询改动无关。
...
### E055 — 被动 effect 让圆圈场景状态在快速反向后过期写回

- 📅 2026-09-24 / P11-I4
- 😱 现象：离开首页的 0/120/260/520ms 采样中 `scenePresence` 始终为 1；快速返回首页后，旧 archive 更新才把它从 1 跳到 0，再缓慢回到 1。
- 🧠 原因：场景 presence 的目标切换由被动 `useEffect` 启动；路由转场和主线程任务密集时，effect 与旧 RAF 回调可能晚于新相位执行，且回调没有用相位/目标版本拒绝过期写入。
- 🔧 修复：改为绘制前同步目标的 `useLayoutEffect`，并给动画回调加 generation 保护；旧 RAF 不匹配当前目标时立即退出。
- ✅ 结果：硬件 Edge 的逐帧、快速反向和 reduced-motion Gate 通过；完整 verify 通过，进入 I5。
- 💡 跨路由动画不能只检查最终稳定值；必须逐帧采样并让每个异步写入证明自己仍属于当前目标。

### E056 — 强制 SwiftShader 让逐帧动画 Gate 退化到每秒约一帧

- 📅 2026-09-24 / P11-I4
- 😱 现象：代码修复后，0/120/260/520ms 证据仍显示 presence 整段不动，最终稳定相位才跳到端点；宽松脚本一度把它误判为通过。
- 🧠 原因：无界面 Edge 被测试工具强制使用 SwiftShader，同时运行完整双 Canvas 水塘，只能约每秒绘制一帧；产品 RAF 与诊断 dataset 都没有中间帧。
...
### E059 — 页眉转场链接替换时误删共享 Link 导入

- 📅 2026-09-23 / P11 本地切场闪烁修复
- 😱 报错：ESLint 提示 `PondHeader.tsx` 中 `Link` 未定义。
- 🧠 原因：品牌标题改用 `PondRouteLink` 后，误以为文件不再使用 `Link`；艺术家导航仍依赖它。
- 🔧 修复：保留 `next/link` 导入，仅让首页品牌链接走池塘转场。
- ✅ 结果：相关文件 ESLint 与差异检查通过。
- 💡 替换共享组件里的单个链接前，先核对该组件内其他链接是否仍使用原实现。

### E060 — 档案预备层提前可点导致冷路由快速返回卡在退场

- 📅 2026-09-23 / P11 本地预备层试验
- 😱 现象：自动浏览器在 `/me` 地址尚未落定时点“返回池塘”，URL 仍为 `/`，但画面停在 `leaving-home`。
- 🧠 原因：预备层比路由先显现，返回链接已可点；控制器把当前地址等于目标地址当成普通同页导航，未取消前一个 `/me` 意图。
- 🔧 修复：档案层到 `/me` 稳定态前保持 inert；存在未完成意图时同地址点击走反向转场，并记录起点以纠正迟到的旧导航。
- ✅ 结果：本地无登录浏览器连续两轮 `/ → /me → /` 收敛到正确 URL/阶段，档案始终只有一份且 Canvas mount ID 不变。
- 💡 预加载前景可以先可见，但交互所有权必须等路由身份确认后才交接。

### E061 — 把导航意图 ref 直接暴露给渲染触发 React 规则报错

- 📅 2026-09-23 / P11 本地预备层试验
- 😱 报错：ESLint `react-hooks/refs`：`Cannot access refs during render`。
- 🧠 原因：为区分 `/me` 与 `/score` 退场目标，组件渲染时读取了 `intentRef.current`。
- 🔧 修复：保留 ref 处理导航时序，另用 React state 向视觉层发布目标路由；两者在点击、取消与收场时同步更新。
- ✅ 结果：TypeScript 与定向 ESLint 通过。
- 💡 控制流可用 ref，但渲染需要可订阅的 state，不能把 ref 当作 UI 状态源。

### E062 — 开发服务器生成的路由类型文件写入不完整

- 📅 2026-09-23 / P11 圆圈退场修复
- 😱 报错：TypeScript 在 `.next/dev/types/routes.d.ts` 和 `validator.ts` 报 `TS1128`。
- 🧠 原因：开发服务器生成的临时类型文件尾部出现重复残片；业务源码与本次类型改动无关。
- 🔧 修复：停止开发服务器，仅移除两份损坏的生成文件，重新运行类型检查；源码和配置未改。
- ✅ 结果：TypeScript 与定向 ESLint 通过；本地预览可重新启动。

### E063 — 我的唱片接口无法区分两条曲目关联

- 📅 2026-09-23 / P11 本地档案读取修复
- 😱 报错：`/api/me/score-nfts` 返回 500，数据库报 `PGRST201`；`/me` 显示“唱片档案读取失败”。
- 🧠 原因：P15 新增 `score_queue_base_identity_fk` 后，`score_nft_queue` 到 `tracks` 有两条外键，原查询 `tracks(title)` 变得不明确。
- 🔧 修复：明确使用原有 `score_nft_queue_track_id_fkey`，不改变唱片或测试链数据。
- ✅ 结果：同一数据库的指定关联查询、TypeScript 与定向 ESLint 通过。

### E064 — Score 首播音效绕过高速镜像

- 📅 2026-09-23 / P11 本地播放修复
- 😱 现象：`/score/1` 显示三个 Arweave 网关超时，播放暂不可用。
- 🧠 原因：本地没有镜像配置，且通用资源解析器只读调用参数、未兜底读取已配置的公开镜像地址；首窗音效因此只尝试 5 秒超时的网关。
- 🔧 修复：为本地 3000 配置公开镜像，解析器在未显式覆盖时使用该地址；保留 SHA-256 校验和网关回退。
- ✅ 结果：定向测试、类型检查与浏览器 cold/hot 首声排程均通过，实际资源请求命中高速镜像。

### E065 — Score 转场成功后遗留 deadline timer 与取消轮询 RAF

- 📅 2026-09-24 / P11-I8 生命周期总验收
- 😱 现象：连续往返审查发现，每次已成功结束的 Score View Transition 仍会保留 31 秒 deadline；异常或快速反向时，来源 ID 未变化可让取消检测 RAF 持续运行。
- 🧠 原因：`Promise.race` 只等待首个结果，没有保存和释放另外两条异步分支的 timer/RAF 句柄。
