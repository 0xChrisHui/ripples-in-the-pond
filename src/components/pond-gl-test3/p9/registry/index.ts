...
if (
  P9_EFFECTS.length !== VALID_SOUND_KEYS.length
  || new Set(P9_EFFECTS.map((effect) => effect.id)).size !== VALID_SOUND_KEYS.length
) {
...
const invalidSoundKeys = P9_ACTIVE_EFFECTS
  .map((effect) => effect.soundKey)
  .filter((key) => !isSoundKey(key));
const missingSoundKeys = VALID_SOUND_KEYS.filter((key) => !EFFECT_BY_SOUND_KEY.has(key));
if (
  EFFECT_BY_SOUND_KEY.size !== VALID_SOUND_KEYS.length
  || invalidSoundKeys.length > 0
  || missingSoundKeys.length > 0
) {
  throw new Error(
    `[P9] 效果键必须与共享 33 键注册表完全一致`
      + `（缺失: ${missingSoundKeys.join(',') || '无'}；非法: ${invalidSoundKeys.join(',') || '无'}）`,
  );
}
...
export function findP9Effect(audioKey: string): P9EffectDefinition | null {
  const normalizedKey = audioKey.toLowerCase();
  return isSoundKey(normalizedKey) ? EFFECT_BY_SOUND_KEY.get(normalizedKey) ?? null : null;
}
