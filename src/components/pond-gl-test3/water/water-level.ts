...
// /test3 task 4：水面**固定**在中段 0.5（球靠浮沉在其上下穿越，不再靠滚轮升降水位）。
// 页面不挂 useWaterLevelControl → stepWaterLevel 不跑 → current 恒 0.5；滚轮改归 pointer-fx 控透视缩放。
let target = 0.5;
let current = 0.5;
let lastWheelAt = -9999; // 指示器淡入用：最近一次滚轮的时间戳（performance.now）

/**
 * 「有效水位」域端点：原始水位 current∈[0,1] 是 UI/缩放/指示器用的归一控制量，但球 z∈[0,1] 配上
 * 没入阈值（+0.02/0.12 软带）会让 z≈1 的球在 current=1 时仍没全淹、z≈0 的球在 current=0 时仍带水。
 * 故把 current 线性映射到「有效水位」EFF_LOW..EFF_HIGH 喂给所有"没入判定"（shader 没入遮罩/深度、
 * getSubmerge、depthFactor、浮沉焦点）→ current=0 全部完全出水（悬空）、current=1 全部完全没入。
 * 缩放/水线/指示器仍读原始 current（getWaterLevel）→ 二者解耦，互不影响。
 */
const EFF_LOW = -0.15;  // current=0：低于最低球(z=0)的没入软带 → 连 z=0 也完全出水
const EFF_HIGH = 1.20;  // current=1：高于最高球(z=1)的没入软带 → 连 z=1 也完全没入

/** shader 缩放/水线/指示器读的「原始水位」current（已缓动，∈[0,1]）。 */
