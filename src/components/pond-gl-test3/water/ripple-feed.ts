...
function trailDrops(nodes: GlPhysNode[], w: number, h: number, t: RippleTuning, proj: ProjCtx): Drop[] {
  const out: Drop[] = [];
...
/** 对象喂涟漪总入口：拖球尾迹 + 穿越溅起（含 >6 合并）。proj：把球落点投影到屏幕(贴视觉球)+用渲染深度判穿越。 */
export function collectObjectDrops(nodes: GlPhysNode[], w: number, h: number, t: RippleTuning, proj: ProjCtx): Drop[] {
  return [...trailDrops(nodes, w, h, t, proj), ...splashDrops(nodes, w, h, t, proj)];
}
