...
export default function WaterDistort(
  { debug = false, glSim, sphereDrift = false, depthModel = false, sphereShadow = false, shadowOcclude = false, shadowGlow = false, shadowContact = false, caustics = false, waterZoom = false, pondFloor = false, moonReflect = false }:
  { debug?: boolean; glSim?: GlSim; sphereDrift?: boolean; depthModel?: boolean; sphereShadow?: boolean; shadowOcclude?: boolean; shadowGlow?: boolean; shadowContact?: boolean; caustics?: boolean; waterZoom?: boolean; pondFloor?: boolean; moonReflect?: boolean },
) {
  ...
  useFrame((state) => {
    ...
    applySpheres(composite, nodes ?? EMPTY_NODES, size.w, size.h, getEffectiveWaterLevel(), getWaterLevel(), glSim?.playingIdRef.current ?? null, proj);
