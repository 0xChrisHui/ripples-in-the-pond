...
/**
 * 把"没入边界(submerge=0.5)"对齐到深度 lineDepth：建点后调用 → 默认水线落在球深度**中位数**，
 * 让球在水线上下对半分；任一方向滚轮(/test1 升降水位)都立即让球穿越水面 = 零死区(无需先空滚一段)。
 * submerge=0.5 ⇔ z = effective−0.04 ⇔ effective = lineDepth+0.04 ⇔ current 反解。直接置 target=current(无缓动跳变)。
 */
