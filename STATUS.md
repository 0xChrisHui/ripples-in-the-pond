...
**P15-G 生产实测优化（2026-09-15）**：首轮 Production 的 Blob Range 真实 TTFB 约 `0.824–0.976s`，旧“探针 + 完整镜像共用 900ms”会安全回退，但几乎不给镜像完整 GET 机会，7.3MB 底曲尤其无法命中。首版竞速 Production 进一步证明 800ms 抢跑会让 Arweave 在 Blob 探针完成前获胜；最终按实测把合同校准为：resolver 起点 1.2 秒启动 Arweave 备用、Range 探针最多 2 秒、镜像完整 GET 最多 10 秒；镜像与单条顺序 Arweave 分支以完整类型/长度/SHA 验证结果竞速，胜方立即取消败方。下一步为重跑完整 Gate、Production 重发与真实播放复验。
