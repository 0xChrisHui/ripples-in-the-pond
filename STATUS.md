...
**当前权威下一步**：将当前 P16 分支合入 `main` 后执行 P16-F Ethereum Mainnet Gate：部署主网 ScoreNFT、显式注册 `(chainId 1, contract, next_token_id 1)`、配置生产环境并完成第一枚低成本真实 mint。主网部署前不得复用 Sepolia 编号，也不得在未注册计数器时开放入口。
