...
**Phase**: **Phase 11-I「主线重放与效果对齐」暂停在 I1-a Gate**（2026-09-24 夜间执行）。I0 自动基准已提交；I1-a 的一行外键修复仍在工作区。完整 Gate 的构建阶段因本机 Google Fonts 网络请求连续三次失败，按夜间规则停止；I2–I8 未开始。P15-I5 暂列开放项。
...
**P15-I2/I3 启动闭包与播放意图（2026-09-21）✅**：HTML 预取底曲与前 8 秒使用音效；播放器完整校验该启动闭包后即可开播，其余音效以 4 路并发后台补齐，极慢时在事件前暂停而不漏音。`loading` 按钮现可记录或取消首次播放意图，AudioContext 仍只在用户手势中创建；资源到齐后自动解码排程，无需二次点击。定向测试覆盖 8 秒边界、后台资源不阻塞、单次 AudioContext 与排队自动播放。

**P15-I4 条件流式底曲（2026-09-21）✅**：I3 Preview 的点击到首声 cold p95 `3717ms`、hot p95 `2087ms`，按 Gate 进入 I4。底曲改从不可覆盖的 Verified Edge URL 直接流式播放，以媒体时钟驱动音效/P9；Edge 流失败时才下载、完整校验并解码永久底曲回退。I4 Preview `dpl_AeuctvwFSWz2Ejep2BQPxVMBDPzw` 20/20 有效、0 console/page error，cold 点击到首声 p95 `1775ms`、启动闭包 p95 `365ms`，达到冷播放 ≤2 秒；hot p95 `1814ms` 仍受 HKG Blob 首包延迟限制，未达到 500ms 进阶预算。页面加载静音预热实验因与启动闭包争抢连接、显著降低成功率而已独立回退，不进入发布。
...
**P15-G 生产实测优化（2026-09-15）**：首轮 Production 的 Blob Range 真实 TTFB 约 `0.824–0.976s`，旧“探针 + 完整镜像共用 900ms”会安全回退，但几乎不给镜像完整 GET 机会，7.3MB 底曲尤其无法命中。首版竞速 Production 证明 800ms 抢跑会让 Arweave 在 Blob 探针完成前获胜，因此校准为 1.2 秒；第二轮发现 Vercel Blob 虽在网络层正确返回 Content-Range，却未用 CORS 暴露给页面 JS，最终运行时以 `206 + Length 1 + 音频类型 + 实际 1 byte` 准入，不可见 Range 由发布 Gate 严核，可见但错误仍拒绝。最终部署 `dpl_4XXpeA2Y9oKfsHK4czSbVH8N8bEC` READY 并指向正式域名；`/`、`/score/1`、`/echo/1`、`/api/tracks` 均 200。独立冷浏览器实证 Score 与 ECHO 都先发严格 Range、再发完整 Blob GET：Score 底曲 7,326,016 bytes，ECHO 首片 126,402 bytes；屏蔽 Blob 后 ECHO 四个首窗片段均从 ArDrive 返回 200/126,402 bytes。完整 `scripts/verify.sh` 通过（37 路由、Foundry 56/56），P15-G 封存。
...
**P15 首屏原始圆二次修复（2026-09-17）**：线上逐帧确认闪现来源不是 WaterDistort 的 Canvas 帧，而是页面级 `z-10` CSS fallback 圆越过了被限制在 GL `z-0` 内的旧遮罩。首页现从首个客户端页面帧挂载 `z-25` 启动遮罩，并在场景就绪前不挂可聚焦的 DOM 圆；健康水面完成两帧合成或可用 fallback 明确接管后才撤罩，context lost→restore 仍会重新等待合成。TypeScript、定向 ESLint、文件/目录/危险代码 Gate 与 Foundry 56/56 通过；Webpack 已完成代码编译，本地末段仅因隔离工作树无 Supabase 环境变量停止，交由 Vercel 干净环境完成最终 Production Gate。
...
**当前权威下一步**：恢复可下载 Google Fonts 的构建网络，在当前工作区重跑 `bash scripts/verify.sh`；Gate 通过后提交 I1-a 并继续 P11-I1-b。I0 用户确认与日食肉眼目验列在晨报；详见 `reviews/2026-09-24-p11-i-overnight-report.md`。P15-I5 暂缓，不在本轨推送、合并或部署。
