...
**Phase**: **Phase 15-I「Instant Start」执行中**。P15-H 的 Permanent Core 与 Verified Edge 保持不变；本轨只缩短数字 Score 的页面与首声关键路径，先完成 ownerOf 解耦、资源优先级和播放意图排队，只有同口径冷播放 p95 仍超过 2 秒才进入流式底曲。
...
**当前权威下一步**：执行 P15-I5：快进推送 `main`，等待 Production Ready 后在正式域名复测 Score #1–#4，并封存 completion review。
