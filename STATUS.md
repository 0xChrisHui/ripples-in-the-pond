...
**下一步** —— Track A 继续推进：
  1. **A9 vercel-env-sync 脚本**（key 差异对比，防 env typo）
  2. **A10 /score 链上灾备 UI 降级壳**（Supabase 抖动时不 404）
  3. **A13 useEventsPlayback 首播 decode 时序**（audioReady / decodeReady 拆分）
  4. **A16 operator-lock Lua 心跳 + fail-closed**
  5. **A17 mint_events upsert 失败检查**
...
**已实质完成的步骤**：
- Phase 7 Track A：A1 chain-config 抽单一来源 ✅（commit `e0084db`）/ A2 AirdropNFT v2 加 `_uriSet` + 重新部署 ✅（2026-05-15）/ A3+A12+A8 score queue 状态机修复包 ✅（2026-05-16，commit `a48f4de`，端到端真实数据验证）/ A6.1+A6.2 tracks 5→15 球 + 10 首 Arweave 上链 ✅（2026-05-16）/ A4 MAINNET-RUNBOOK grantRole 验收 ✅ / A14 ScoreCard 时间 + polling ✅ / A15 useMintScore 60s 回滚 ✅（2026-05-16）
...
- Phase 7 Track D：D1 LoginModal 默认 Semi + 邮箱直跳 Privy ✅ / D2 SemiLogin 两阶段布局 + 6 格 PinInput ✅ / D3 /me 顶栏社区钱包外链 ✅ / D4 verify.sh ✅，浏览器 7 步实测待用户确认（2026-05-15）
