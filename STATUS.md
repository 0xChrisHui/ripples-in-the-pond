...
**当前权威下一步**：用户创建 Public Vercel Blob Store、上传精确 `<base>/<arweave-txid>` 路径的 Score/Pond Echo 音频并配置三环境 `NEXT_PUBLIC_MEDIA_MIRROR_BASE_URL`。Development 本地 pull 后重启，Preview/Production 分别部署并完成 Range/CORS 与自动回退/恢复实测。此后额度耗尽与恢复均不需要改变量或按月重新部署。
