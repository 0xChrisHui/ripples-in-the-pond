...
> - **Pass B' 折射重做（验收追加）**：用户反馈鼠标狂晃/微波下折射碎成麻点破洞。根因 = 合成 pass 的 `-normalize(法线).xz` 让**任何涟漪都满幅位移**。改为**位移 ∝ 高度梯度(坡度) + clamp ±0.025**：平水≈0、缓坡轻折、陡坡强折但不破洞；月光高光改按坡度方向 + gmag 门控。`perturb` 字段改名 `refract`（标度变、默认 0.6、滑块 0–3，老存档自动回默认）；常驻微波改宽而轻。
