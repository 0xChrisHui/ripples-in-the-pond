...
### H4 — 涟漪交互全集 + 限流　✅ 已完成（commit `191d34a`）

> **实现实况**：sim shader 单滴 `uMouse/uStrength` → `uDrops[12]` 数组（一帧多滴），新增 `water/ripple-feed.ts` 统一收集对象喂涟漪。拖球尾迹（被拖球移动 >16px 落一滴）/ 穿越溅起（没入度穿 0.5 阈值）/ 单帧 >6 颗穿越合并成质心一道大涟漪（防水花风暴）/ 常驻微波（每 ~14 帧随机轻滴，ambient=0 关）。三档参数（拖尾/溅起/常驻微波）进 `RippleSpikePanel`；切组 `resetRippleFeed` 清穿越记忆；`bg-ripple:wave` 桥保留（groupWave/hoverRipple 自动接入）；RttSpike 同步迁到 `uDrops[0]` 单滴。
...
### H6 — 参数板 + 控制台开关 + 打磨　✅ 已完成（A `baddee8` + B `a170029`）（H 线总验收）

> **实现实况**：
> - **Pass A `baddee8`**：面板重叠修复（右下角 调色+波纹/运动 改同一 flex 栏从下往上堆叠、永不重叠；水位指示右缘→左缘让位）；`波纹/运动参数` 板分「波纹(9)+运动(浮沉幅度/频率/焦点露出 3)」两节，H5 写死的浮沉常量升格为 `ripple-tuning` 的 bobAmp/bobScale/focusMargin；`ScenePanel` 加「遮罩调试」开关；新增 `reduced-motion.ts`（prefers-reduced-motion 时关球自漂+常驻微波，保留用户主动涟漪）。
> - **Pass B `a170029`**：H3 挂起的高折射色斑 → 高度场 `NearestFilter→LinearFilter` 双线性平滑（梯度连续、消色斑 + 缓解 half-float 量化）。**未升 512²**（会改波速/阻尼观感、推翻已调手感；色斑根因是采样不是分辨率）；sim 只在 texel 中心采样 → Linear 等价 Nearest → 波动方程不变。
> - **需求规格 v1 逐条对照**：1/3/4/5/8/9 ✅ 全达；2/6/7/10 主体 ✅。
> - **明确 deferred 的 4 项 gap（非丢弃，留后续拍）**：① pinch（触控板/触屏）控制 + ② 性能降级参数（高度场分辨率/DPR 面板可调）→ **挂移动端那拍**（规格 7「桌面优先、移动未来再议」）；③ 默认水位滑块进面板（水位现可滚轮+左缘指示控制）→ 小项、按需补；④ 每实例参数 min/max + 随机种子面板（`lw` 已是每实例随机参数，部分满足）→ 挂 I 线组件接入时一起。
