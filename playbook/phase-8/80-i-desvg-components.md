...
> **属 Phase-8-G 伞下**。**红线 / 架构蓝图 / 编号系统 / 执行模式 / 弃用记录**见总文档 [`70-g-gl-water.md`](./70-g-gl-water.md)。H 线（水面子系统）见 [`74-h-water-surface.md`](./74-h-water-surface.md)。
...
- 📦 范围：按用户想法逐个新组件（鱼 / 贴图组件 / 新动效…），各挂 **H5 运动框架**（声明 层/层带 + 运动公式，水上/水下视觉自动；公式签名契约见 [`74-h-water-surface.md`](./74-h-water-surface.md) H5）。
