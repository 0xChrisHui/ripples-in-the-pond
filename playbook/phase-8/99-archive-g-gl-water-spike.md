> # ⚠️ 旧版存档（spike 时期，2026-06-12 ~ 06-13）
> 本文件是 P8-G 的**历史快照**，已被新版三件套取代，**保留供复盘对比、不再据此执行**：
> - 新版 G 线 + 总伞 → [`70-g-gl-water.md`](./70-g-gl-water.md)
> - 新版 H 线（水面子系统）→ [`74-h-water-surface.md`](./74-h-water-surface.md)
> - 新版 I 线（去 SVG + 新组件）→ [`80-i-desvg-components.md`](./80-i-desvg-components.md)
>
> **新旧差异（复盘看点）**：本旧版 = **spike 框架**（G1–G7 线性 + P2"对等 SVG" + G7"去留拍板" + 水位"折射三态：折射≤8px+变暗+青蓝tint"）；新版 = **正式重建**（GL 方向已定、水面改"全屏动态扭曲 RTT"、P2→I 线"去 SVG 重设计"、取消拍板、水下"不变暗"）。下面是旧版原文 ↓
