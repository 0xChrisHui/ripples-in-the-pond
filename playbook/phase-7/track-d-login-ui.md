...
> - Semi 登录的用户在 `/me` 右上角看到「↗ 社区钱包」按钮，外链到 `https://semi.ntdao.xyz/`
>
...
按用户答案"Semi 登录时替换「← 首页」":
- `authSource === 'semi'` → `<a href="https://semi.ntdao.xyz/" target="_blank" rel="noopener noreferrer">↗ 社区钱包</a>`
...
`https://semi.ntdao.xyz/` 走**硬编码常量**，不进 env：
...
- [ ] Semi 用户进 /me：顶栏右上是「↗ 社区钱包」→ 新窗口 https://semi.ntdao.xyz/
...
6. **验证** → modal 关 → /me 顶栏右上**显示「↗ 社区钱包」** → 点击新窗口打开 `https://semi.ntdao.xyz/`
