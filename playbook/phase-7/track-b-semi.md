...
### D-B7 — Track B 与 A1 串行

B4 端到端铸造经 operator-wallet.ts，A1 改 chain 配置期间 Track B 接 e2e 会撞半成品。**B4 必须在 A1 完结后开始**。B1-B3（前端 + env）可在 A1 进行中并行做。
...
| [B4a](#step-b4a--端到端技术冒烟) | 发码 → 登录 → 拿 evm → 铸造 1 个 MaterialNFT 进 Semi 钱包 | 半天 | B3 + A1 | Semi + OP Sepolia + Vercel preview |
...
- **不**新建 LoginEntry / client-jwt 多余文件（控 8 文件硬线）
...
- **不**新建 `src/lib/auth/client-jwt.ts`（JWT 解码 1 行 atob，直接放 useAuth）
...
A1 chain 配置已完结（按 D-B7），B1-vercel 已生效。
