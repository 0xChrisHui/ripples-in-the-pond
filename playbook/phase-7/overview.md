...
> ⚠️ **本文为已完结 Phase 7 的归档 playbook（2026-05-16 完结）**。文中对 Phase 8/9/10 的前瞻定义是 2026-05-13 当时口径，**已被 2026-06-04 新路线图覆盖**（P8 改水塘视觉、UI 大升级挪 P11、主网挪 P12 等）。后续 phase 当前权威以 `playbook/roadmap-P8-P16.md` 为准，本文内 P8+ 描述仅作历史参考。
>
...
| Phase | 内容 | 工时估 |
|---|---|---|
| **Phase 7（已完结 ✅）** | 修严重 BUG + Semi 社区钱包接入 + 全站提速 | 9-12 天 |
...
| **[C](./track-c-perf.md)** | 全站提速 | 7（C1/C2/C3/C5/C6/C7/C8）| 3-4 天 | C2 已吸收为体感目标 + ROI；C1 修前 baseline + C8 修后对照 |
| **[D](./track-d-login-ui.md)** | 登录 UI 抄社区钱包 + Semi 默认 + /me 跳社区钱包按钮 | 4（D1/D2/D3/D4）| 1.3 天 | 前置 Track B B2/B3 已完成；与 Track A 剩余 step 完全可并行 |

**总工时**：14-18 天（三 track 有依赖 + 部分串行；review 修订后比原 9-12 天乐观估更真实）+ Track D 1.3 天（2026-05-15 用户新增）
...
## Track D — 登录 UI 抄社区钱包 + Semi 默认 + /me 跳社区钱包按钮（详见 `track-d-login-ui.md`）

### 用户原话拆解（2026-05-15 新增）

> "登录界面用 references/community wallte 的 UI 设计方案。因为这个项目也有一个登录浮窗，可以直接抄袭 / 登录界面默认基于社区钱包的手机号登录 / 如果是社区钱包登录，在 /me 界面右上角增加一个社区钱包跳转按钮，可以跳转到 https://semi.ntdao.xyz/ / 补充：点击邮箱登录直接跳转 privy，节省异步跳转步骤"

### 拍板的 4 个视觉/UX 选项（用户 2026-05-15 preview 答案）

1. **保留深色，抄结构**（modal 仍 zinc-950，仅抄 semi-app 的纵向骨架：返回箭头 + 大标题 + 副标题 + 全宽 input + 全宽按钮 + 底部辅助链接）
2. **6 格分离 PIN 框**（对齐 semi-app UPinInput；抽 `PinInput.tsx` 组件控行数）
3. **保留全站 modal**（不拆 /login 路由；4 个 caller 不动；D-B6 沿用）
4. **Semi 登录时替换「← 首页」为「↗ 社区钱包」**（互斥，不挤位；外链 `https://semi.ntdao.xyz/`，`target="_blank" rel="noopener noreferrer"`）
5. **附加**：点击底部「邮箱登录」直接弹 Privy 原生 modal（移除当前"用邮箱登录"中间按钮）

### Step 总览

| ID | 内容 | 工时 |
|---|---|---|
| **D1** | LoginModal 抄结构 + 移除 tab + 默认 Semi + 邮箱直跳 Privy | 0.3 天 |
| **D2** | SemiLogin 两阶段重布局（标题/副标题/全宽 input/全宽按钮）+ 抽 PinInput（6 格 + 自动跳格 + 粘贴分发 + iOS OTP）| 0.5 天 |
| **D3** | `app/me/page.tsx` 顶栏按 `authSource` 切换「← 首页」/「↗ 社区钱包」（useAuth 已暴露 authSource，不扩 hook）| 0.2 天 |
| **D4** | 7 步浏览器端到端实测 + commit | 0.3 天 |

**Track D 工时**：1.3 天

### 起点选择 + 并行性

- **D1 / D3 完全独立**，任一可起手；AI 默认建议 **D3 起手**（5 分钟见效果、风险最低）
- Track D 与 Track A 剩余 step（A3+A12 / A4 / A5 / A6 …）**完全可并行**（不碰 operator-wallet.ts / chain-config）
- Track D 不依赖 A1，也不依赖 C3
...
- Semi 社区钱包 deep-link 集成 → P10 视用户量评估
...
→ 用户在意 **"页面打开速度 + 切页体感"**。2026-05-15 已进一步拆成四个体感目标：
- `/` → `/me` 到所有内容展示约 2 秒
- `/me` 待铸造项目"正在上传"约 3 秒
- 点击某个唱片约 1 秒内进入或出现明确反馈
- 进入 `/` 约 0.8 秒内出现明确反馈

执行准则：对这四个目标帮助大、框架改动小、风险低就做；帮助小、改动大、风险高就不做。
...
| **C2** | 四个体感目标 + ROI 准则已拍板（不再单独阻塞）| 已完成 |
| **C3** | /api/me/scores 拆 split（直接命中 `/me` 首屏 + "正在上传"）| 半天 |
| **C4** | /api/me/score-nfts 改用 generated event_count（重叠 A11，已修，audit only）| 已完成 |
| **C5** | 首页 Archipelago 慢网占位加 spinner + 重试按钮（命中 `/` 0.8s 反馈）| 半天 |
| **C6** | next/font 温和优化（display/preload audit；不做激进字体裁剪）| 30 分 |
| **C7** | 路由 transition smooth（loading.tsx 分页骨架，命中唱片 1s 反馈）| 1 天 |
| **C8** | 再跑 Lighthouse + 手动体感对比，出报告 | 半天 |
...
- [ ] **Track D**：4 step 全 ✅ + 用户浏览器 7 步实测全过（登录 modal 默认 Semi + 6 PIN + /me 顶栏外链）
