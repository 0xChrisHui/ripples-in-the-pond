...
> "登录界面用 references/community wallte 的 UI 设计方案。因为这个项目也有一个登录浮窗，可以直接抄袭 / 登录界面默认基于社区钱包的手机号登录 / 如果是社区钱包登录，在 /me 界面右上角增加一个社区钱包跳转按钮，可以跳转到 https://app.ntdao.xyz/ / 补充：点击邮箱登录直接跳转 privy，节省异步跳转步骤"
...
4. **Semi 登录时替换「← 首页」为「↗ 社区钱包」**（互斥，不挤位；外链 `https://app.ntdao.xyz/`，`target="_blank" rel="noopener noreferrer"`）
