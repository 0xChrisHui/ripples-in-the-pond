...
### D-A6 — A6 先做资产盘点，缺曲必须 fail-fast

`public/tracks/` 当前仅 6/108 mp3（001 + No.1-5）。**playbook 假设的"103 曲待上链"是错的**。A6 实施流程：

1. 用户先盘点 102 首曲源（艺术家提供 mp3）
2. AI 校验 108 = 实际文件数；缺即 fail-fast 报错列出缺曲清单
3. 文件齐了再批量上传（每 20 首一批，失败列表续传幂等）
4. UPDATE tracks.arweave_url 后抽样 5 首验证

A6 工时含"等用户补曲"buffer。如果艺术家无法及时提供 102 首，A6 拆 A6.1（先上传已有的）+ A6.2（等齐再补）。
...
| [A6](#step-a6--108-曲-arweave_url-全量上链已资产盘点) | P0 | **先资产盘点** + 缺文件 fail-fast；当前仅 6/108 mp3，需用户先补齐文件 | 1-1.5 天（含等用户补曲）| A5 | Arweave + Base | 硬阻塞 #2 |
...
## Step A6 — 108 曲 arweave_url 全量上链（已资产盘点）

### 概念简报

`tracks.arweave_url=NULL` 的曲目无法走草稿铸造（stepUploadMetadata 必崩）。

**⚠️ playbook 原假设修正**：实际 `public/tracks/` **只有 6 个 mp3**（001.mp3 + No.1.mp3 ~ No.5.mp3），不是 5/108。**102 首曲源缺失**。A6 必须先做资产盘点，缺曲必须 fail-fast。

### 📦 范围（环境：Arweave + Base credits + DB）
- `scripts/arweave/upload-tracks.ts`（已存在，需加 fail-fast 缺文件检查）
- `scripts/arweave/audit-tracks.ts`（**新建**，108 vs 实际文件对照报告）
- DB 操作：`UPDATE tracks SET arweave_url = ... WHERE id = ...`

### 做什么

**A6.0 资产盘点（必先）**：
1. AI 跑 `audit-tracks.ts` 输出三类清单：
   - 已有 mp3 + DB 有 arweave_url（不动）
   - 已有 mp3 + DB 无 arweave_url（待上链）
   - DB 有记录但 mp3 缺失（用户必须先补曲）
2. **如果第 3 类不为零 → 停下让用户先补曲**（艺术家未给 → A6 拆 A6.1 先上传已有的 + A6.2 等齐再补）

**A6.1 已有曲上传**：
3. upload-tracks.ts 改批量循环 + 每 20 首一批 + 失败列表续传幂等（不静默跳过）
4. 跑脚本 → 收集 txid → 回写 DB

**A6.2 缺曲补齐**（异步等用户）：
5. 用户补齐 mp3 → 再跑一次脚本上传剩余

### 验证标准

- [ ] `audit-tracks.ts` 报告无第 3 类（"DB 有 mp3 缺失"）
- [ ] `SELECT COUNT(*) FROM tracks WHERE arweave_url IS NULL` = 0
- [ ] 抽样 5 首：DB `arweave_url` 与 Arweave 上的 mp3 一致（fetch HEAD 返 audio/mpeg）
- [ ] 跑一次端到端草稿铸造（任意一首未在 B6 demo 5 球内的）确认 stepUploadMetadata 不崩
- [ ] 用户补曲与上传分离：若 102 首暂时只补 50 首，A6.1 完结允许部分进度，但 P7 完结 gate 必须 108/108
...
- [ ] A6 108/108 曲 arweave_url 齐全（含艺术家补曲完成）
