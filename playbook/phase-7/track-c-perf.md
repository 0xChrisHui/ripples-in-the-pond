...
### D-C0 — baseline 不在 A3+A12 完结前跑

A12 lease 25 分钟问题不修，"上链中"草稿状态会拉长 LCP 感知。C1 baseline 数据会被污染。**C1 必须等 A3+A12 修复包完结**。Track C 其他 step（C3 / C5 / C6 / C7）可与 A 并行。
...
四个页面的 LCP / FCP / TTI 目标值不预设（C2 step 用户拍板）。AI 提供行业建议（首页 LCP < 2.5s / 内页 < 3s），但用户基于"投资人 demo 节奏" + "艺术家审美"决策。
...
**关键顺序**：A11+A12 完结前不跑 C1 baseline（避免数据被 25min lease 状态污染）；C3 必须先于 A14+A15（DraftCard polling 契约由 C3 定义）。

| Step | 内容 | 工时 | 依赖 | 与其他 track 重叠 | 环境 |
|---|---|---|---|---|---|
| [C1](#step-c1--lighthouse-baseline) | Lighthouse 跑 4 个核心页面 baseline | 半天 | **A3+A12 必须先完结** | 无 | Vercel preview |
| [C2](#step-c2--目标值讨论) | 看 baseline 报告 + 用户拍板每页目标 | 30 分（讨论）| C1 | 无 | — |
| [C3](#step-c3--api-me-scores-拆-split) | 拆 /api/me/scores：首屏返主信息，events 单独 endpoint | 半天 | 无 | **必须先于 A14+A15** | 本地 |
| [C5](#step-c5--首页慢网占位--spinner--重试) | "正在唤醒群岛"加 spinner + 网络较慢提示 + 手动重试 | 半天 | 共享原子组件 | **复用 = LoadingSpinner + RetryButton 两原子组件**（不复用业务壳）| 本地 |
| [C6](#step-c6--字体加载优化) | next/font Modak + Azeret 优化 | 30 分 | 无 | 无 | 本地 |
| [C7](#step-c7--路由-transition--loading-tsx-骨架) | /me, /score/[id], /artist 加 loading.tsx 骨架页 | 1 天 | C5（共享 LoadingSpinner）| 复用 C5 原子组件 | 本地 |
| [C8](#step-c8--lighthouse-对比报告) | 再跑 Lighthouse 对比 baseline 出报告 | 半天 | C3-C7 + 重测 buffer | 无 | Vercel preview |

**Track C 总工时**：4-5 天（含 C8 发现未达标 → 部分返工 buffer；未达标项可挪 P8 与 UI 翻修一起做）
...
### 📦 范围
- `reviews/2026-05-XX-phase-7-perf-baseline.md`（新建，存 baseline 数据）
...
行业经验值（Google Core Web Vitals）：
- LCP < 2.5s = Good / 2.5-4s = Need Improvement / > 4s = Poor
- FCP < 1.8s = Good / 1.8-3s = Need Improvement
- TTI < 3.8s = Good / 3.8-7.3s = Need Improvement
...
AI 拿 C1 baseline 报告 → 给 3 档建议（保守 / 中等 / 激进）→ 用户拍板每页目标。

### 验证标准

- [ ] 4 页 × 桌面/移动 = 8 个目标值固定
- [ ] 写进 `reviews/2026-05-XX-phase-7-perf-baseline.md` 作为 C8 对比基准
...
重跑 C1 同样 8 组测试，对比 baseline + 对照 C2 目标值：
- 全部达标 → Track C 完结
- 部分达标 → 评估剩余 gap 是否挪 Phase 8（UI 翻修一起做）
...
- [ ] C8 对比报告显示 4 页 LCP / FCP 达 C2 目标值（至少 ≥ 80%）
- [ ] 用户实测体感快
