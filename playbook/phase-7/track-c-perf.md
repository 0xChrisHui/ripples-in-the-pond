...
**关键顺序**（2026-05-13 修订）：C1 现在跑作为"修前 baseline"（不再等 A3+A12）；C3 必须先于 A14+A15（DraftCard polling 契约由 C3 定义）；C8 在 A3+A12 + C3-C7 完结后跑作为"修后对照"。

| Step | 内容 | 工时 | 依赖 | 与其他 track 重叠 | 环境 |
|---|---|---|---|---|---|
| [C1](#step-c1--lighthouse-baseline) | Lighthouse 跑 4 个核心页面"修前 baseline" | 半天 | **无（Track C 起点）** | 无 | Vercel preview |
...
**2026-05-13 修订**：C1 现在跑作为**修前 baseline**（不等 A3+A12），即使含 25min lease 影响"上链中"草稿状态感知，也反映当前用户真实体验。C8 修后对照报告对比这组数据 + C2 目标值。
