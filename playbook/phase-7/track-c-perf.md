...
| [C9](#step-c9--me--scoreid-首屏内容轻量化) | 解决页面壳出现后真实内容仍慢的问题 | 1-2 天 | C3-C8 + 用户实测仍慢 | 无 | 本地 |

**Track C 总工时**：约 4-6 天（C2 不再单独占时；C9 是 C8 后用户实测仍慢时追加的延伸 step。继续遵守高性价比边界：先做 `/me`，再判断 `/score/[id]` 是否进入 P7）。
...
| /score/[id] SSR 拉完整 events_data | 已修（C9b：首屏改 event_count，events 走 client-side fetch） | ✅ C9b |
| `/me` 页面壳已快但唱片/草稿内容仍需约 2 秒 | 已修（C9a：三 fetch 并行 + section 独立 error / skeleton） | ✅ C9a |
| `/score/[id]` 进入后详情内容仍需约 2-3 秒 | 已修（C9b R1 cache() dedupe + R2 不阻塞 events_data + R3 events 按需 fetch） | ✅ C9b |
...
## Step C9 — `/me` + `/score/[id]` 首屏内容轻量化

> **触发原因**：C3-C8 后，路由反馈已经变快，但用户实测 `/me` 的唱片/待铸造内容仍需约 2 秒展示，`/score/[id]` 的详情内容仍需约 2-3 秒展示。C9 解决的是“页面壳出现后，真实内容仍慢”的第二层问题。

### 概念简报

C7 的 `loading.tsx` 解决的是“切页有没有反馈”，不是“真实数据何时展示”。C9 要把首屏必须展示的轻数据和播放/events/metadata 等重数据拆开，并检查请求是否并行。原则是先让基础内容快出来，重数据按需加载或后台补齐。

### 执行顺序

1. **C9a：先做 `/me` 内容区首屏轻量化**。
2. **C9b：只有 C9a 后 `/score/[id]` 仍明显影响 demo，再决定是否继续做**。
3. 如果 C9b 需要大改 Server Component / 数据源架构，停下说明，默认挪 Phase 8 / Phase 10。
...
先诊断，实际修改只限命中慢点的文件：
- `app/me/page.tsx`
- `src/components/me/ScoreNftSection.tsx`
- `src/components/me/DraftCard.tsx`
- `src/components/me/ScoreCard.tsx`
- `app/api/me/score-nfts/route.ts`
- `app/api/me/scores/route.ts`

如实际文件名不同，以代码现状为准；不要为追求计划一致性强行新建抽象。

#### 做什么

1. 诊断 `/me` 当前数据链路：确认 `/api/me/score-nfts` 和 `/api/me/scores?light=1` 是并行还是串行，确认慢在接口还是前端等待结构。
2. 如果当前是串行请求，改为并行请求，避免唱片和草稿耗时叠加。
3. 给 `/me` 内部的唱片区和草稿区补 section-level skeleton：页面壳进入后，每个区域独立显示加载态、空态、错误态。
4. audit `/api/me/score-nfts` 首屏字段：首屏只应需要 id / title / cover / status / createdAt / track title / tokenId / eventCount；如仍拉完整 events / metadata / 链上历史，再考虑 `?light=1`。
5. 保持 C3 契约：`/api/me/scores?light=1` 不重新拉回 `events_data`，DraftCard 播放仍按需 fetch events。

#### 验证标准

- [x] DevTools Network 里 `/api/me/score-nfts` 与 `/api/me/scores?light=1` 如需同时展示，应尽量并行，不串行叠加。
- [x] `/me` 页面进入后，唱片区和草稿区都有独立 skeleton，不再空白等待。
- [x] 一个 section 慢或失败，不阻塞另一个 section 展示。
- [x] `/api/me/scores?light=1` 继续不拉 `events_data`。
- [x] `/api/me/score-nfts` 经 audit 后确认首屏无重 JSON（commit `0d75a93` migration 031 已完成 P1-19，本轮无需再改）。
- [x] 用户本地实测进入"我的音乐"后，约 1 秒内看到基础卡片或明确的区域骨架反馈。
- [x] `bash scripts/verify.sh` 通过。

---

### C9b — `/score/[id]` 详情页首屏轻量化候选

#### 📦 范围（环境：本地；默认先诊断）

- `app/score/[id]/page.tsx`
- `app/score/[id]/ScorePlayer.tsx`
- `src/data/score-source.ts`
- 如确需新增 endpoint，再考虑 `app/api/score` 或现有 API 子目录，先确认目录硬线。

#### 做什么

1. 诊断 `/score/[id]` 当前是否由 Server Component 一次性 await 基础信息、events、metadata、cover、链上 fallback。
2. 区分首屏必须数据和重数据：标题 / 封面 / track / status / tokenId 属于首屏；events / player data / metadata fallback / 链上灾备属于重数据。
3. 优先尝试让 `ScorePlayer` 在用户点击播放时再加载 events，而不是详情页首屏阻塞。
4. 如需要拆 `getScorePreview(id)` 与 `getScoreEvents(id)`，先保持 token id / queue id 双路由兼容，不破坏现有分享页。
5. 如果发现必须重构 score-source 或公开详情页数据模型，停下让用户确认是否挪 Phase 8 / Phase 10。

#### 验证标准

- [x] 点击唱片后 100-300ms 内仍有 C7 骨架反馈。
- [x] 详情页基础信息不被 events 大 JSON 阻塞（首屏改 SELECT `event_count`，events 走 `/api/scores/[id]/events` client-side fetch）。
- [x] 播放按钮首次点击可以加载 events 并播放（ScorePlayer 三态：loading / error / ready），失败时显示红条 + "请稍后刷新"。
- [x] token id / queue id 双兼容不回归（`getScoreEvents` 复用同口径路由解析）。
- [x] `/score/12` 不出现 500；若仍有稳定性问题，归 A10/P8/P10 继续跟（本轮未引入新 500 路径）。
- [x] `bash scripts/verify.sh` 通过。
...
- [x] C1-C9 状态 ∈ {fixed, downgraded-accepted, moved-to-P8/P10}（C1-C8 已收口；C9a + C9b 全做且本地实测通过）
- [x] C8 对比报告显示四个体感目标明显改善；LCP / FCP 至少作为佐证，不作为唯一完结标准
- [x] C9a 后用户实测 `/me` 内容区不再空白等 2 秒
- [x] C9b 已做：events 大 JSON 不再阻塞首屏 HTML，ScorePlayer 改挂载时 fetch
- [x] STATUS.md 悬空 TODO 里"/api/me/scores 35s 慢"已划掉（C3 解决）
