...
| A5 | 输入键 ↔ 音效 id 解耦 | **登记挂未排期音效扩展**，本 track 不做 | — |
...
- 未来扩至 50 时需要：键盘键→音效 id 映射层（录制写入 id）+ 新输入方案（26 键装不下 50）
- **为什么主网前不做**：事件逐 NFT 钉死 + §4 规则已消除永久性风险（老事件 a-z 永远有效）；
  主网前不动录制主链路 = 少一分上线风险。
  （2026-07-11 评估修正：此前会话曾倾向“现在做”；2026-09-05 因 P15 改为全站性能，本项退回未排期）
...
3. **a-z 保留**：首批 26 个 id（a-z）永久保留；未来扩容至 50 时新音效一律用新 id
