...
| ✅ B-0 | `20-b` | **已拍板 2026-07-19** | 九项全定，见 `20-b` §1 顶部锁定块（不做版税/不设上限/印死/加幂等键+空串防御+freezeURI/名 RPIP/admin 独立钱包/不部署空投=3 合约） |
| 🛑 B-1 | `20-b` | CT 修完 | 测试网重部署回归验收 |
| ✅ C-0 | `30-c` | **已拍板 2026-07-19** | 三项全定：Semi 维持现状 / 备份=手动导出+部署日快照 / 额度不升级（见 `30-c` §0 锁定块） |
...
## 8. 当前执行路径（2026-07-19 定，B-0/C-0 已锁，A/B 代码已完成）
...
3. **A-1 解码器 UI 优化轮**（用户提出 2026-07-19）→ 浏览器验收 → Arweave 上传 →
   `SCORE_DECODER_AR_TX_ID` 三环境切换（txid 未定稿前不切）
4. **D-1 内容冻结**：正式曲名落库（占位名清零）+ admin 独立钱包创建 + deployer
   一次性钱包创建充值 + DB 快照 ≤24h
5. **🛑 D-0 排期拍板** → **D 部署日**（整块 2-3h，用户亲手广播）→ 🛑 D-gate 开铸
6. **E 性能**（软 gate）随时穿插，不阻塞上述任何一步
