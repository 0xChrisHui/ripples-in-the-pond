...
- [x] 首次全量快照 ✅ 2026-07-27：**338 行 / 14 张表** → `C:\Users\Hui\ripples-backups\20260727-154332`
      （仓库外）。方式=service-role key 走 PostgREST 全表导出 JSON（本机无 pg_dump），
      结构靠 `supabase/migrations/`（git）+ 数据靠 JSON = 完整可恢复。**部署日前 24h 再做一份**
- [x] 备份产物存放与恢复步骤 ✅ 已写进 `docs/MAINNET-RUNBOOK.md` **§9**（含表清单/依赖序/生成列坑）
- [x] **恢复演练 ✅ 2026-07-27 通过**：自建测试行走完整闭环（造行→捕获→删除→确认消失→
      从备份恢复→逐字段比对一致→清理），**不碰用户数据**。结论 restore path PROVEN
