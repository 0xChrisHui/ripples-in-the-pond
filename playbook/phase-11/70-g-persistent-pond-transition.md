...
- 共享 Shell 在后台挂载唯一 `MeArchivePage` 并预备私人档案；`/me` 只切换可见性与交互权，不重挂实例、不重复请求。
- 未认证状态不发私人请求；账号变化用 `authSource + userId + wallet` 身份代次、abort 与缓存隔离使旧结果失效。
- 私人数据全部成功或明确失败前，导航保持当前稳定前景；隐藏档案必须 inert、`aria-hidden`，且不接受 pointer、键盘或焦点。
