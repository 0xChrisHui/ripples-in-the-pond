...
export async function POST(request: Request, { params }: { params: Promise<{ orderId: string }> }) {
  ...
  const { data } = await supabaseAdmin.from('score_self_mint_orders')
    .select('status').eq('order_id', orderId).eq('user_id', auth.userId).maybeSingle();
  if (!data) return Response.json({ error: '订单不存在' }, { status: 404 });
  if (data.status !== 'preparing_assets') return Response.json({ accepted: false, status: data.status });
  after(async () => {
