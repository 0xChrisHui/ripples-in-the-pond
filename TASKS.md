...
- **Phase 8 进行中**：Wave 0+1 已合并入 main（35 个新 flag 默认 false）。两条待办线由用户定先后：
  ...
  - ② **P8-G /test1 GL 渲染层 spike**（2026-06-12 立项，**进行中**）：**G1-G5 ✅**（克隆+关透视+GL 基调骨架 / G4 GL 球 instanced+DOM 命中层+TunePanel `744b64d` / G5 程序化水面+视觉控制台 `eb5e11d`）；**G6-1 已实现**（水位 store+滚轮缓动+wheelMode 互斥+球淡出覆盖+水位指示，工作树）。**2026-06-14 方向再拍板 + 重组 G/H/I 三线**（水面改"动态扭曲"对标 jquery.ripples + 水位全局变量 + 鼠标交互；G 地基✅ / H 水面子系统 / I 去SVG+全新组件，G7 拍板取消、收口并入 I）。**H1–H6 ✅ = H 线全完**（RTT spike `a8b8adb` / 扭曲整合真场景 `7bd678c` / 水位深度遮罩 `1210c4c` / 涟漪交互全集 `191d34a` / 对象运动模型 `2065e25`+`9892be1` / H6 总验收[参数板布局+运动参数+reduced-motion+色斑修复 Nearest→Linear] `baddee8`+`a170029`）。规格 v1 主体全达；deferred 4 项（pinch/性能降级参数→移动端、默认水位滑块→按需、每实例 min/max+种子→I 线）。**I 线**：I1 ✅（去 SVG/卸 Archipelago+停 sim+GlNav `7469ae4`）+ I2 ✅（GL 日蚀 `add5a2f`）+ I3 ⏸ 暂缓（新组件待用户定）。**方向已定：GL 替换 SVG 首页**→ 新立 **J 线（生产化迁移）`playbook/phase-8/phase-8-j-migration.md`**：**J1 ✅**（WebGL 兜底 `4abc882`+修复 `967b66a`：不白屏 + Canvas 全程只挂一次防 context 泄漏）/ J2 手机+resize 不错位 / J3 自动降配 / J4 加载提示+重试+预热；deferred 已铸造记号/生产化/定稿/正式替换+删 SVG。**下一步 = J2**。新版见 `playbook/phase-8/phase-8-{g,h,i,j-migration}.md`
